...
void f(TreeNode* root, int targetSum, int& count) {
    if(root == NULL) return;
    else if(targetSum == root->val) count++;

    targetSum -= root->val;
    f(root->left,targetSum,count);
    f(root->right,targetSum,count);
    targetSum += root->val;
...
int main() {
    ...
    int targetSum = 22;
    int paths = pathSumIII(root,targetSum);
    cout << "Number of paths are -> " << paths << endl;
}   