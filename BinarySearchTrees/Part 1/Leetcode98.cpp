...
void inOrderTraversal(TreeNode* root, vector<int>& space, bool& answer) {

    if(root == NULL) return;

    inOrderTraversal(root->left,space,answer);
    
    if(space.size() == 0) space.push_back(root->val);
    else {
        int k = space.size();
        if(space[k-1] > root->val) {
            answer = false;
            return;
        }
    }
    space.push_back(root->val);
    inOrderTraversal(root->right,space,answer);
...
bool isValidBST(TreeNode* root) {
    vector<int> v;
    bool answer = true;
    inOrderTraversal(root, v, answer);
